...
-- VERSIÓN CORREGIDA: Tipos BIGINT para compatibilidad con JPA/Long
-- ==========================================================
...
CREATE TABLE usuarios_sistema (
    id_usuario_sis BIGINT AUTO_INCREMENT PRIMARY KEY,
...
SELECT id_usuario_sis, nombre, username, rol FROM usuarios_sistema;

SELECT '--- Libros disponibles ---' AS '';
SELECT l.id_libro, l.titulo, l.cantidad_disponible, a.nombre AS autor 
FROM libros l 
LEFT JOIN autores a ON l.id_autor = a.id_autor;

SELECT '--- Clientes ---' AS '';
SELECT id_cliente, dni, CONCAT(nombre, ' ', apellido) AS nombre_completo, 
       CASE WHEN activo = 1 THEN 'Activo' ELSE 'Inactivo' END AS estado 
FROM clientes;

SELECT '--- Préstamos ---' AS '';
SELECT p.id_prestamo, l.titulo, CONCAT(c.nombre, ' ', c.apellido) AS cliente, 
